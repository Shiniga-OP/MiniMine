...
public class BlocoBau {
	...
	public static Item[] obterOuCriar(int x, int y, int z) {
		final long ch = Chave.gerar(x, y, z);
	...
	public static void aoDestruir(int x, int y, int z) {
		...
		for(int i = 0; i < inv.length; i++) {
			if(inv[i] != null && inv[i].quantidade > 0) {
				final ItemMundo drop = new ItemMundo(inv[i].nome, inv[i].quantidade, x, y, z);
				drop.posicao.set(x + 0.5f, y + 0.5f, z + 0.5f);
				Mundo.entidades.add(drop);
			}
		}
	}
	// === interface ===
