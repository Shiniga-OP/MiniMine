...
public class Mundo {
    public final int CHUNK_TAMANHO = 16; // padrao: 16, testes: 8
    public final int MUNDO_LATERAL = 64; // padrao: 64, testes: 32
    public int RAIO_CARREGAMENTO = 7; // padrao: 8, testes: 1
	...
	public Bloco[][][] gerarChunk(final int chunkX, final int chunkZ) {
		...
		for(int x = 0; x < CHUNK_TAMANHO; x++) {
			...
			for(int z = 0; z < CHUNK_TAMANHO; z++) {
				int globalZ = baseZ + z;
				// gerar temperatura e umidade
				float temp = (PerlinNoise2D.ruido(globalX * 0.004f, globalZ * 0.004f, seed + 1000) + 1) / 2;
				float umidade = (PerlinNoise2D.ruido(globalX * 0.004f, globalZ * 0.004f, seed + 2000) + 1) / 2;
				int biomaAtual;
				if(temp < 0.25f) biomaAtual = MONTANHA; // areas frias
				else if(temp < 0.6f) {
					if(umidade < 0.3f) biomaAtual = DESERTO;
					else if(umidade < 0.6f) biomaAtual = PLANICIE;
					else biomaAtual = FLORESTA;
				} else {
					if(umidade < 0.4f) biomaAtual = DESERTO;
					else biomaAtual = LAGO; // areas quentes e úmidas
				}
				biomas[x][z] = biomaAtual;
				Bioma b = BIOMAS.get(biomaAtual);
				float ruido2D = PerlinNoise2D.ruidoFractal2D(
					globalX, globalZ, 
					b.escala2D, seed, 
					4,       // 4 camadas de ruído
					0.5f     // Persistência
				);
				alturas[x][z] = (int) (b.altBase + ruido2D * b.variacao);
			}
		}
		// gera os blocos
		for(int x = 0; x < CHUNK_TAMANHO; x++) {
			...
			for(int z = 0; z < CHUNK_TAMANHO; z++) {
				...
				for(int y = 0; y < MUNDO_LATERAL; y++) {
					...
					if(y == 0) tipoBloco = "BEDROCK";
						...
						float ruido3D = PerlinNoise3D.ruidoFractal3D(
							globalX * b.escala3D, y * b.escala3D, globalZ * b.escala3D, 
							seed + 100, 
							3,      // 3 camadas
							0.5f    // persistencia
						);
						if(ruido3D > b.caverna) tipoBloco = "AR";
						else tipoBloco = b.blocoCaver;
			...
			for(int y = 0; y < MUNDO_LATERAL; y++) {
				for(int z = 0; z < CHUNK_TAMANHO; z++) {
					...
					for(int face = 0; face < 6; face++) {
						...
						List<float[]> lista = dadosPorTextura.computeIfAbsent(atlasTexturaId, new Function<Integer, List<float[]>>() {
								@Override
								public List<float[]> apply(Integer k) {
	...
	public void definirBiomas() {
		// Planície (mais plana)
		BIOMAS.add(new Bioma(22f, 3f, 0.03f, 0.14f, "GRAMA", "TERRA", "PEDRA", 0.12f));

		// Deserto (pequenas dunas)
		BIOMAS.add(new Bioma(20f, 5f, 0.04f, 0.1f, "AREIA", "AREIA", "PEDRA", 0.01f));

		// Montanha (grandes variações)
		BIOMAS.add(new Bioma(30f, 20f, 0.02f, 0.16f, "PEDRA", "PEDRA", "PEDRA", 0.15f));

		// Floresta (moderado)
		BIOMAS.add(new Bioma(22f, 4f, 0.05f, 0.1f, "GRAMA", "TERRA", "PEDRA", 0.15f));

		// Lago (muito plano)
		BIOMAS.add(new Bioma(18f, 0.1f, 0.001f, 0.12f, "AGUA", "AREIA", "PEDRA", 0.1f));

		// Floresta com Lagos
		BIOMAS.add(new Bioma(22f, 3f, 0.05f, 0.1f, "GRAMA", "TERRA", "PEDRA", 0.15f));
	}
