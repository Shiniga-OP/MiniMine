...
public class GLRender implements GLSurfaceView.Renderer {
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {  
		...
		int raio = mundo.RAIO_CARREGAMENTO;
		chunksTotais = ((raio * 2 + 1) * (raio * 2 + 1));
    } 
	
	public Runtime rt;
	public float livre, total, usado;
	public int frame = 0, chunksTotais = 0;
	...
	public void propagarAgua() {
		executorFisica.submit(new Runnable() {
				public void run() {
							...
							for(int y = 0; y < mundo.MUNDO_LATERAL; y++) {
								for(int z = 0; z < mundo.CHUNK_TAMANHO; z++) {
									...
									for(int i = 0; i < 4; i++) {
										...
										if(viz == null || viz instanceof Agua) {
											Agua vizi = (Agua) viz;
											if(!(viz instanceof Agua) || vizi.nivel >= 7) {
					...
					for(ChunkCoord c : chunksAfetados) if(!mundo.chunksAlterados.containsKey(c)) mundo.chunksAlterados.put(c, true);
				}
			});
	}
    ...
    public void atualizarChunks() {
		int chunkJogadorX = (int) player.pos[0] >> 4;
		int chunkJogadorZ = (int) player.pos[2] >> 4;
		...
		for(int x = chunkJogadorX - mundo.RAIO_CARREGAMENTO; x <= chunkJogadorX + mundo.RAIO_CARREGAMENTO; x++) {
			for(int z = chunkJogadorZ - mundo.RAIO_CARREGAMENTO; z <= chunkJogadorZ + mundo.RAIO_CARREGAMENTO; z++) {
				...
				if(!mundo.chunksAtivos.containsKey(chave)) {
					float dist = (float) Math.hypot(x - chunkJogadorX, z - chunkJogadorZ);
		...
		if(livre >= 10.0 || !trava || total <= 30.0) {
			while(!fila.isEmpty() && carregados < chunksPorVez) {
				...
				if(mundo.chunksAtivos.containsKey(chave)) continue;
	...
	public static class ChunkCandidato {
		...
		int x, z;
		float distancia;
		ChunkCandidato(ChunkCoord chave, int x, int z, float distancia) {
			this.chave = chave;
			this.x = x;
			this.z = z;
	...
	public void salvarMundo(OutputStream saida, int seed, Map<ChunkCoord, Bloco[][][]> chunksCarregados) throws IOException {
	...
	public void carregarMundo(InputStream entrada, Mundo mundo) throws IOException {
		...
		for(int i = 0; i < totalChunks; i++) {
			...
			for(int k = 0; k < totalNaoAr; k++) {
				...
				Bloco b = mundo.criarBloco(globalX, y, globalZ, id);
				if(id.equals("AGUA")) {
					Agua a = (Agua) b;
					a.nivel = dis.readInt();
					a.dir = dis.readInt();
...
class ChunkCoord {
    ...
    public boolean equals(Object o) {
        ...
        ChunkCoord isso = (ChunkCoord) o;
        return x == isso.x && z == isso.z;
    }
    @Override
    public int hashCode() {
        return 31 * x + z;
